...
# Generate Kubernetes user certificate on the Kubernetes cluster
function create_user_cert {
    local USER=$1
    local GROUP=$2

    # Set paths for PKI and kubeconfig directories
...
function create_kubeconfig {
    ...
    kubectl config set-context ${USER}-context --cluster=kubernetes --namespace=${NAMESPACE} --user=${USER} --kubeconfig=${KUBECONFIG_DIR}

    echo "Kubeconfig created for user $USER in group $GROUP"
}
...
for USER in $(yq e '.users[].name' $CONFIG_FILE); do
  ...
  create_user_cert $USER "users"  # User certificates are saved in /etc/kubernetes/pki/users directory
...
for GROUP in $(yq e '.groups[].name' $CONFIG_FILE); do
  ...
  for USER in $(yq e ".groups[] | select(.name == \"$GROUP\") | .users[]" $CONFIG_FILE); do
    ...
    create_kubeconfig $USER $GROUP  # Create kubeconfig in group-specific subdirectory
